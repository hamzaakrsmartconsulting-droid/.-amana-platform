...
// Page UI Raqîb — pattern identique à Sajl/Mawsim avec libellés conformité.
...
export default function RaqibPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      const res = await fetch('/api/agents/raqib', {
        method: 'POST',
      ...
      while (true) {
        ...
        for (const evtBlock of events) {
          ...
          if (event === 'text_delta' && typeof (data as { text?: string }).text === 'string') {
            ...
            toolCalls.push(data as { name: string; input: unknown })
            setMessages((prev) => {
              const next = [...prev]
              const last = next[next.length - 1]
              if (last && last.role === 'assistant')
            ...
            toolResults.push(data as { name: string; result: unknown })
            setMessages((prev) => {
              const next = [...prev]
              const last = next[next.length - 1]
              if (last && last.role === 'assistant')
            ...
            setError((data as { message?: string }).message ?? 'Erreur Raqîb')
          }
        }
      }
    } catch (e) {
      ...
      <header className="mb-4 flex items-center justify-between border-b border-amana-grey-light pb-3">
        <div>
          ...
          <p className="text-sm text-amana-grey">
            RCCI · LCB-FT · criblages · audit dossier · échéances cabinet
          </p>
        </div>
        <Link
          href="/admin/dossiers"
      ...
      <main className="flex-1 space-y-3 overflow-y-auto pr-2">
        {messages.length === 0 && (
          <div className="rounded border border-amana-gold bg-amana-cream p-4 text-sm text-amana-dark">
            <p className="font-semibold text-amana-forest">Que dois-je auditer ?</p>
            <ul className="mt-2 list-disc pl-5">
              <li>« Audit conformité du dossier [id] »</li>
              <li>« Quelles alertes critiques ouvertes ? »</li>
              <li>« Enregistre criblage PEP clean pour ce dossier, source ComplyAdvantage »</li>
              <li>« Liste les échéances cabinet à venir »</li>
              <li>« Crée une alerte critical : RC pro AIG échue 28/02/2026 »</li>
            </ul>
          </div>
        )}
        {messages.map((m, i) => (
